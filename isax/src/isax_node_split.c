...
void split_node(isax_index *index, isax_node *node)
{
    ...
    if (node->sax_buffer_size > 0)
        for (i = node->sax_buffer_size - 1; i >= 0; i--)
        {
            ...
            split_buffer[split_buffer_index].sax_lower = node->sax_lower_buffer[i];
            split_buffer[split_buffer_index].sax_upper = node->sax_upper_buffer[i];
            split_buffer[split_buffer_index].start_pos = node->start_pos_buffer[i];
            split_buffer[split_buffer_index].min_len = node->min_len_buffer[i];
            split_buffer[split_buffer_index].window_size = node->window_size_buffer[i];
            split_buffer_index++;
        }
    ...
    if (node->prev_sax_buffer_size > 0)
        for (i = node->prev_sax_buffer_size - 1; i >= 0; i--)
        {
            ...
            split_buffer[split_buffer_index].sax_lower = NULL;
            split_buffer[split_buffer_index].sax_upper = NULL;
            split_buffer[split_buffer_index].start_pos = NULL;
            split_buffer[split_buffer_index].min_len = NULL;
            split_buffer[split_buffer_index].window_size = -1;
            split_buffer_index++;
        }
    ...
    if (node->filename != NULL)
    {
        ...
        if (prev_sax_file != NULL)
        {
            ...
            while (!feof(prev_sax_file))
            {
                ...
                split_buffer[split_buffer_index].sax = malloc(sizeof(sax_type) * index->settings->paa_segments);
                split_buffer[split_buffer_index].sax_lower = NULL;
                split_buffer[split_buffer_index].sax_upper = NULL;
                split_buffer[split_buffer_index].start_pos = NULL;
                split_buffer[split_buffer_index].min_len = NULL;
